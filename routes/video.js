...
router.get('/:id/stream', async (req, res) => {
  ...
  // Free videos: no login required at all
  if (video.accessType === 'free') {
    return res.json({ url: getSignedUrl(video.videoUrl) });
  }

  // Subscription/pay-per-view videos: require login
  const authHeader = req.headers.authorization;
  if (!authHeader || !authHeader.startsWith('Bearer ')) {
    return res.status(401).json({ message: 'Login required to watch this video.' });
  }
  let userId;
  try {
  ...
  if (video.accessType === 'pay_per_view') {
    ...
    video.views += 1;
    await video.save();
    return res.json({ url: getSignedUrl
